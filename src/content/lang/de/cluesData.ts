/**
 * German crossword clues: short DEFINITIONAL German clue + its English
 * equivalent (the same idea in both languages) — NOT the literal translation,
 * so the answer isn't given away. Keyed by lemma id. Authored for the early
 * blocks' crossword words; later words fall back to the plain English gloss.
 *
 * (Moved verbatim out of `content/clues.ts`, which is now the language-agnostic
 * dispatcher.)
 */
import type { Clue } from '../../clues';

export const CLUES_DE: Record<string, Clue> = {
  // ── pronouns ──────────────────────────────────────────────────────────────

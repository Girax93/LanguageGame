...
## Languages: multi-language architecture (German + Norwegian + French)

The app ships **three course languages** — German (`de`, the default), Norwegian Bokmål (`no`) and
French (`fr`) — chosen from the in-app **language menu** (Home → Learn). Each was added without
touching the games: everything below the content layer is shared and language-agnostic. French is
**shipped, pushed to `main`, and live on Vercel**; it is a polished **~420-lemma core** for now (42
blocks) — Ari's "polished core first" call — not the full ~2000 yet.
...
- **Language packs: `src/content/lang/`.** `types.ts` is the `LangPack` interface (code, name, flag,
  ...
  versions). `de/`, `no/` and `fr/` each hold `alphabet.ts`, `grammar.ts`, `cluesData.ts`, `index.ts`; `no/` and
  `fr/` also have their own `lemmas.*.ts` (`lemmas.no.ts` = 2000 Norwegian lemmas; `lemmas.fr.ts` = ~420
  French) and `cipher.ts` (the per-language sentence generator). `fr/index.ts` exports the French `LangPack`.
...
- **French specifics.** Gender is the TWO-gender system **un/une** (masc./fem.) — no neuter, so every
  ...
  cipher sentences instead. Alphabet is A–Z plus the accented letters **É È Ê À Â Ç Î Ô Û Ù**, each a
  distinct cipher/Hurdle letter (like German's ä/ö/ü/ß); the keyboard is **QWERTY + a top accent row**
  (Ari's call, not AZERTY), which needed a small N-row generalisation of `HurdleKeyboard` (it now renders
  all-but-last rows plain + the last row with Enter/⌫ — behaviour-identical for German/Norwegian). The
  cipher (`lang/fr/cipher.ts`) uses a full **être person table** (je suis / tu es / il est / …), enforces
  predicate-adjective **gender agreement** via a curated `ADJ_FEM` table (grand→grande), wraps negation as
  **ne … pas** (elided to n' before a vowel), builds identity with **C'est un/une …**, gives transitive
  verbs the post-verbal pronoun **ça** ("Il voit ça."), derives 3rd-person-singular present from
  `forms[0]`, and elides le/la→l'. Curated sets keep it grammatical (no colours/dimensions on people, no
  size adjective on mass nouns, mass/abstract nouns kept out of "c'est un/une"). Crossword clues are
  authored in `lang/fr/cluesData.ts` (French definition + English). French invariants live in
  `tests/content.fr.mts`.
...
- **"Hurdle" game — ✅ SHIPPED** (was here). Built as a gating 4th Practice game + free Recap; see
  "Hurdle specifics" and the Curriculum gate above.
...
- **Expand French toward ~2000 lemmas.** French currently ships a polished ~420-lemma core (42
  blocks); grow it the way German/Norwegian were built, keeping the cipher's curated agreement tables
  (`ADJ_FEM`, etc.) in sync as new adjectives/verbs are added.
...
- **German, Norwegian + French all shipped** — the app is multi-language now (see *Languages* above).
  French (`LANGS = [de, no, fr]`) is a ~420-lemma polished core (42 blocks), pushed to `main` and live on
  Vercel; expand toward ~2000 later.

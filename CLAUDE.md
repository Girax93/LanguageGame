...
The Practice session has **two gating halves**: the grammar drills above (`practiceCounts`, X/3)
and the **generated letter-cipher session** (`cipherCounts` — solve every sentence the block
generates; together they cover all its new words). So `isBlockComplete = isBlockLearned &&
blockPracticeDone && cipherSessionDone`. The crossword challenge will rejoin later. Recap mode
(grammar + cipher) replays learned material full-pool without driving the gate.
...
- **Letter ciphers are GENERATED** from the lemmas (`generateCipher.ts` → `cipherItems.ts`). Per
  block we generate the small set of grammatically-safe sentences whose words together cover the
  block's newly-learned words (`cipherItemsForBlock` / `cipherRoundsForBlock`). The generator only
  emits provably-correct constructions (nominative article+noun, predicate adjectives, `sein` +
  3rd-person-singular verbs from `forms[0]`, locative/PP predicates, coordinating/subordinating
  joins, questions); anything it can't place safely falls back to a single-word puzzle, so coverage
  is always 100%. Sentences get richer as vocabulary grows.
- **The block gate now requires BOTH practice halves**: the grammar drills (`blockPracticeDone`,
  X/3) AND the cipher session (`cipherSessionDone` — all of the block's generated sentences solved,
  tracked per block in `state.cipherCounts`, shown as a sentences progress bar). See `isBlockComplete`.
- **Recap crosswords are still temporarily empty** — `crosswords.ts` ships `[]` while its generator
  is rebuilt. The crossword challenge capstone is still dormant (unwired).
- Articles `der/die/das` are their own early entries glossed "the (masc./fem./neut.)".
- `STATE_VERSION = 7` (old saves reset). `tests/content.invariants.mts` asserts the lemma-dataset
  + gating invariants — sets/lookups, generated grammar drills, **generated cipher coverage (every
  block's session covers its new words; requires are real + eligible; unique ids; levels in range)**,
  empty crossword, the per-block Practice gate (grammar `practiceCounts` **+ cipher `cipherCounts`**),
  and the daily recap (`recapDue`/`lastRecapAt`).

// French content + progression invariants. Mirrors content.invariants.mts but
// switches the active language to French first. Run:
...
setActiveContentLanguage('fr');

import {
  SETS, ALL_WORDS, wordById, setIndexForWord,
  germanWithArticle, englishWithArticle, answerMatches,
} from '../src/content/vocab';
import { LEMMAS_FR } from '../src/content/lang/fr/lemmas.fr';
import { CLUES_FR } from '../src/content/lang/fr/cluesData';
...
if (getActiveCode() !== 'fr') fail('active-not-fr', getActiveCode());
if (ALL_WORDS !== LEMMAS_FR) fail('all-words-not-fr');

// (a) dataset shape
{
  const ids = new Set<string>(); let dupes = 0;
  for (const w of LEMMAS_FR) { if (ids.has(w.id)) dupes++; ids.add(w.id); }
  if (dupes) fail('a:dup-ids', dupes);
  const surfaces = new Set<string>(); let dupSurf = 0;
  for (const w of LEMMAS_FR) { if (surfaces.has(w.de)) { dupSurf++; console.log('  dup surface', w.de); } surfaces.add(w.de); }
  if (dupSurf) fail('a:dup-surfaces', dupSurf);
  const orders = LEMMAS_FR.map((w) => w.order).sort((a, b) => a - b);
  for (let i = 0; i < orders.length; i++) if (orders[i] !== i + 1) { fail('a:order-gap', `${orders[i]}!=${i+1}`); break; }
  for (const w of LEMMAS_FR) { if (!w.id || !w.de || !w.en) fail('a:empty', w.id); if (w.de !== w.de.toLowerCase()) fail('a:not-lowercase', w.de); }
}
// (b) gender hygiene: every noun has a gender (m/f only); gender only on nouns.
{
  const PLURAL_ONLY = new Set<string>([]); // no plural-only nouns in the FR core yet
  for (const w of LEMMAS_FR) {
    if (w.gender && w.pos !== 'noun') fail('b:gender-nonnoun', w.id);
    if (w.pos === 'noun' && !w.gender && !PLURAL_ONLY.has(w.de)) fail('b:noun-no-gender', w.de);
    if (w.gender && !['m', 'f'].includes(w.gender)) fail('b:bad-gender (FR is m/f)', `${w.id}:${w.gender}`);
  }
}
// (c) sets + lookups
{
  const flat = SETS.flatMap((s) => s.words);
  if (flat.length !== LEMMAS_FR.length) fail('c:set-count');
  const w = LEMMAS_FR[20];
  if (wordById(w.id) !== w) fail('c:wordById');
  if (setIndexForWord(w.id) < 0) fail('c:setIndex');
}
// (d) grammar drills: un/une by gender, translation "a <en>"
{
  const nouns = LEMMAS_FR.filter((w) => w.pos === 'noun' && w.gender);
  ...
  for (const g of GRAMMAR_ITEMS) {
    ...
    const expected = noun.gender === 'f' ? 'une' : 'un';
...
// (e) cipher: 1..4 sentences/block, eligible requires, >=1 coverage, avg healthy
{
  ...
  const nb = Math.floor(LEMMAS_FR.length / BS);
  let totalCov = 0;
  for (let b = 0; b < nb; b++) {
    const learnedThru = new Set(LEMMAS_FR.slice(0, (b + 1) * BS).map((w) => w.id));
    ...
    const target = LEMMAS_FR.slice(b * BS, b * BS + BS);
...
// (g2) grammar session = the block's DISTINCT learned nouns, never padded.
{
  const B = PROGRESSION.setsPerBlock;
...
// (h) display helpers: "un homme" / "a man"; "une femme"
{
  const noun = LEMMAS_FR.find((w) => w.gender === 'm')!;
  const art = noun.gender === 'f' ? 'une' : 'un';
  ...
  const f = LEMMAS_FR.find((w) => w.gender === 'f');
  if (f && germanWithArticle(f) !== `une ${f.de}`) fail('h:fem', germanWithArticle(f));
}
// (i) articles present: un/une (indefinite) + le/la (definite)
{
  const arts = LEMMAS_FR.filter((w) => w.pos === 'art');
  for (const a of ['un', 'une', 'le', 'la']) if (!arts.find((x) => x.de === a)) fail('i:missing-article', a);
}
...
{
  if (triesFor('un') !== 5 || triesFor('homme') !== 8) fail('m:tries', `${triesFor('un')},${triesFor('homme')}`);
  if (answerLength('ÉCOLE') !== 5) fail('m:len-accent', answerLength('ÉCOLE'));
  const eq = (a: unknown[], b: unknown[]) => a.length === b.length && a.every((x, i) => x === b[i]);
  if (!eq(scoreGuess('CHAT', 'CHAT'), ['correct','correct','correct','correct'])) fail('m:score');
  if (!isSolved('CHAT', 'CHAT')) fail('m:solved');
  ...
  for (let b = 0; b < nb; b++) {
    ...
    totalH += items.length;
    const blockIds = new Set(LEMMAS_FR.slice(b * BS, b * BS + BS).map((w) => w.id));
...
// (n) early-block crossword words carry a real FR + EN definition (not the literal answer).
{
  let checked = 0;
  for (let b = 0; b <= 3; b++) {
    const item = crosswordItemsForBlock(b);
  ...
  console.log(`FR crossword clues checked = ${checked}`);
}
...
console.log(`\nFR: lemmas=${LEMMAS_FR.length} sets=${SETS.length} blocks=${blockCount(SETS)} grammar=${GRAMMAR_ITEMS.length} crosswords=${CROSSWORDS.length} hurdlePool=${HURDLE_ITEMS.length}`);
if (!ok) { console.log('\nFRENCH INVARIANTS FAILED'); process.exit(1); }
console.log('OK — French invariants pass');
